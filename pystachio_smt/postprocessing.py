...
def postprocess(params, simulated=False):
    ...
    if not params.ALEX:
        ...
        for i in range(len(spots)):
            # ***** ADD THIS CHECK *****
            # Only process frames that actually contain spots
            if spots[i].num_spots > 0:
                # Access attributes only if spots exist
                tmp = spots[i].spot_intensity
                tmp_snr = spots[i].snr
                # Check if tmp is not empty before concatenating (optional robustness)
                if tmp.size > 0:
...
# --- Function to calculate Diffusion Coefficient and Localisation Precision ---
def get_diffusion_coef(traj_list, params, channel=None):
    diffusion_coefs = []
    loc_precisions = []

    # --- Accumulators for Average MSD ---
    # Use max possible lags needed based on msd_num_points
    max_lags = params.msd_num_points
    # Sum of MSD values for each lag
    all_msd_sum = np.zeros(max_lags)
    # Count of trajectories contributing to each lag
    all_msd_count = np.zeros(max_lags, dtype=int)
    # Time lag values (assuming constant frame time)
    all_tau = np.arange(1, max_lags + 1) * params.frame_time
    # --- End Average MSD Setup ---

    # Iterate through each trajectory to calculate individual D and accumulate MSD data
    for traj in traj_list:
        trajectory_length = traj.length
        # Skip trajectories shorter than required points for MSD fit
        if trajectory_length < params.msd_num_points + 1:
        ...
        # Initialise arrays for individual trajectory MSD calculation
        MSD = np.zeros(max_lags)
        tau = np.zeros(max_lags)
        track_lengths = np.zeros(max_lags)
        # Get x, y coordinates in physical units (microns)
        x = np.array(traj.path)[:, 0] * params.pixel_size
        y = np.array(traj.path)[:, 1] * params.pixel_size

        # Calculate MSD for different time lags (i) up to max_lags
        for i in range(1, max_lags + 1):
            # Ensure lag does not exceed trajectory length
            if i >= trajectory_length:
                MSD[i - 1] = np.nan # Mark MSD as invalid if lag is too long
                track_lengths[i - 1] = 0
                tau[i-1] = i * params.frame_time
                continue
            ...
            if len(sqd) > 0:
                ...
                track_lengths[i - 1] = len(sqd) # Store number of points averaged
                tau[i - 1] = i * params.frame_time # Store time lag
            else: # Should not happen if loop/length checks are correct
                MSD[i - 1] = np.nan
                track_lengths[i - 1] = 0
                tau[i-1] = i * params.frame_time

        # Filter out any potential NaN values from this trajectory's MSD
        valid_indices = ~np.isnan(MSD)
        tau_valid = tau[valid_indices]
        MSD_valid = MSD[valid_indices]
        track_lengths_valid = track_lengths[valid_indices]

        # Check if enough points remain for fitting this trajectory
        if len(tau_valid) < 2: # Need at least 2 points for a line fit
             # Warning is optional, could be many short trajectories
             # print(f"WARNING: Trajectory ID {traj.id} too short for fitting after processing.")
             continue

        # Calculate weights for fitting (less weight for longer lags with fewer points)
        weights = track_lengths_valid.astype("float32") / float(
            np.amax(track_lengths_valid)
        ) if np.amax(track_lengths_valid) > 0 else np.ones_like(track_lengths_valid)

        # --- Fit individual trajectory for D coefficient ---
        try:
            # Fit a straight line (y = mx + c) to individual MSD vs tau
            popt, pcov = curve_fit(straightline, tau_valid, MSD_valid, p0=[1, 0], sigma=1.0/weights, absolute_sigma=False)

            # Process Fit Results
            if popt[0] >= 0: # Check for physically meaningful slope
                # Calculate Diffusion Coefficient (D = slope / 4 for 2D)
                diffusion_coef = popt[0] / 4.0
                diffusion_coefs.append(diffusion_coef)
                # Calculate Localisation Precision (sigma) if intercept is positive
                if popt[1] > 0:
                    loc_prec = np.sqrt(popt[1]) / 2.0
                    loc_precisions.append(loc_prec)
                # --- Accumulate valid MSD data for averaging ---
                # Add this trajectory's valid MSD values to the sum
                # Need to align based on lag index (i-1)
                valid_lag_indices = np.where(valid_indices)[0] # Get original 0-based indices of valid lags
                all_msd_sum[valid_lag_indices] += MSD_valid
                all_msd_count[valid_lag_indices] += 1
                # --- End Accumulation ---
            # else: # Optional: Warn if slope is negative
                # print(f"WARNING: Fit resulted in negative slope ({popt[0]:.2e}) for trajectory {traj.id}. Skipping.")
        except Exception as e: # Catch any other fitting errors
            print(f"WARNING: Unable to fit curve for trajectory {traj.id}. Error: {e}")
        # --- End individual trajectory fit ---

    # --- Calculate and Plot Average MSD ---
    plt.figure(figsize=(8, 6)) # Create figure for Average MSD plot
    # Calculate average MSD where count > 0, avoid division by zero
    valid_avg_indices = all_msd_count > 0
    avg_msd = np.full(max_lags, np.nan) # Initialise with NaN
    avg_msd[valid_avg_indices] = all_msd_sum[valid_avg_indices] / all_msd_count[valid_avg_indices]
    avg_tau = all_tau # Use the predefined tau values

    # Filter out NaN values from average MSD for plotting and fitting
    plot_valid_indices = ~np.isnan(avg_msd)
    avg_tau_plot = avg_tau[plot_valid_indices]
    avg_msd_plot = avg_msd[plot_valid_indices]

    # Plot the average MSD curve
    if len(avg_tau_plot) > 0:
        plt.plot(avg_tau_plot, avg_msd_plot, marker='o', linestyle='-', label='Average MSD')
        # --- Optional: Fit the average MSD curve ---
        # Note: Fitting the average may give slightly different results than averaging individual fits
        # avg_weights = all_msd_count[plot_valid_indices].astype('float32') / float(np.amax(all_msd_count)) # Weight by number of trajectories per point
        # try:
        #     popt_avg, pcov_avg = curve_fit(straightline, avg_tau_plot, avg_msd_plot, p0=[1, 0], sigma=1.0/avg_weights, absolute_sigma=False)
        #     plt.plot(avg_tau_plot, straightline(avg_tau_plot, *popt_avg), color='red', linestyle='--', label='Fit to Average')
        #     avg_D_from_avg_fit = popt_avg[0]/4.0
        #     print(f"Average D calculated from fit to average MSD: {avg_D_from_avg_fit:.3f}")
        # except Exception as e:
        #      print(f"Warning: Could not fit average MSD curve. Error: {e}")
        # --- End Optional Fit ---
    else:
        plt.text(0.5, 0.5, "No valid MSD data to plot", horizontalalignment='center', verticalalignment='center')

    # Format Average MSD Plot
    plt.xlabel(r"Time Lag $\tau$ (s)")
    plt.ylabel("Average MSD ($\mu$m$^2$)")
    plt.title(f"Average MSD vs Time Lag (n={all_msd_count[0]} trajectories for first lag)") # Indicate N
    if len(avg_tau_plot) > 0: plt.legend()
    # Save Average MSD plot
    avg_msd_filename = params.name + "_avg_MSD_plot.png"
    if channel=="L": avg_msd_filename = params.name + "_Lchannel_avg_MSD_plot.png"
    elif channel=="R": avg_msd_filename = params.name + "_Rchannel_avg_MSD_plot.png"
    plt.savefig(avg_msd_filename, dpi=300)
    if params.display_figures: plt.show() # Show plot if enabled
    plt.close() # Close the Average MSD figure
    # --- End Average MSD Plot ---

    # --- Plot Histogram of Diffusion Coefficients (calculated from individual fits) ---
    plt.figure(figsize=(8, 6)) # Create new figure
    if diffusion_coefs: # Check if any D values were calculated
        plt.hist(diffusion_coefs, bins='auto') # Plot histogram with auto bins
        mean_diff_coeff = np.mean(diffusion_coefs)
        # Create title with mean D and count
        hist_title = f"Diffusion Coefficients\nMean = {mean_diff_coeff:.3f} $\mu$m$^2$s$^{-1}$ (n={len(diffusion_coefs)})" # Increased precision
    else:
        # Display message if no valid D values found
        plt.text(0.5, 0.5, "No valid diffusion coefficients found", horizontalalignment='center', verticalalignment='center')
        mean_diff_coeff = np.nan
    ...
    plot_title_prefix = ""
    if channel=="L":
        plot_title_prefix = "Left channel "
        ofile = params.name + "_Lchannel_diff_coeff.png"
    elif channel=="R":
        plot_title_prefix = "Right channel "
        ofile = params.name + "_Rchannel_diff_coeff.png"
    plt.title(f"{plot_title_prefix}{hist_title}")
    ...
    # --- Save Data to Files (Diffusion Coeffs and Loc Precision) ---
    # Save Diffusion Coefficients
    diff_coeff_file = params.name + "_diff_coeff_data.csv"
    print(f"Saving diffusion coefficients to: {diff_coeff_file}")
    with open(diff_coeff_file, "w") as f:
    ...
    # Save Localization Precisions
    loc_prec_file = params.name + "_loc_precision_data.csv" # Updated filename
    print(f"Saving localization precisions to: {loc_prec_file}")
    with open(loc_prec_file, "w") as f:
        f.write("LocalizationPrecision_um\n") # Header
        for lp in loc_precisions:
    ...
    # Return calculated D and sigma values from individual fits
    return diffusion_coefs, loc_precisions
